...
- **Shadow Raids are remote-raidable.** [INTERNAL REFERENCE ONLY — DO NOT include this in newsletter copy. Per `spawn_point_copy_accuracy.md`, treat remote-raidability as silent default; mentioning it is default-filler. Only flag the exception when a specific Shadow Raid event is explicitly in-person only per Niantic's announcement.]

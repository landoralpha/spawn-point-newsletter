...
| **Reddit `r/<sub>/search.rss?q=<query>&restrict_sr=on&sort=new`** (Atom feed, keyword-scoped) | sandbox-blocked | ✅ 200 (verified 2026-09-07 — real, current, keyword-matched posts) | **fetch_url — used by Step 2c's sentiment/volume sweep to search for community complaint trends instead of hoping the 25-entry snapshot catches them.** |

...
4. **`firecrawl_scrape` / `firecrawl_search` from the Firecrawl MCP** — real browser-rendered fetch with a stealth proxy option. **Verified 2026-09-18: this is a YouTube-only win, NOT a general-purpose upgrade over `fetch_url`.** `firecrawl_scrape` on a `youtube.com/watch?v=...` URL returns full video metadata (title, channel, upload date, view/like counts) AND the complete auto-generated transcript as clean text, for 1 credit, no login — this is the pipeline's only source of YouTube content (there was none before). `firecrawl_search` restricted to `includeDomains: ["youtube.com"]` finds relevant dataminer/creator videos by keyword. Do NOT use Firecrawl for Twitter/X or Reddit: both were tested live 2026-09-18 and both fail outright — Twitter/X returns "all scraping engines failed" (even stealth proxy can't beat the login/JS wall), and Reddit is hard-denylisted ("we do not support this site"). For those two, the existing WebSearch-snippet (Twitter) and `.rss` (Reddit) paths remain the only working ones.
   ...
   **Ethical-use rule (Joe's explicit preference, 2026-09-18): reference and credit YouTube videos, don't repurpose their transcripts as newsletter copy.** The transcript is fetched only to verify a claimed leak/detail is accurate — read it, judge it, then discard it. Never copy or closely paraphrase the creator's own sentences into a Notion field or newsletter draft; write the description in original wording and always name the creator/channel. The video's URL is the citation — it's meant to be linked and credited (per the "source-link titles" convention), not scraped for its speech. See `triggers/monitor.md` Step 2c for the full rule and the `Content Completeness` cap this implies.
...
- Reddit subreddit `.rss` feeds — `r/pokemongo/.rss` and `r/TheSilphRoad/.rss` — Atom feeds, real entries. Latest-post snapshot only (~25 entries), not filtered by keyword.
- Reddit `search.rss` — `reddit.com/r/<sub>/search.rss?q=<query>&restrict_sr=on&sort=new` — same Atom format, but scoped to a keyword query within one subreddit. Verified live 2026-09-07: querying `r/pokemongo/search.rss?q=shiny+odds&restrict_sr=on&sort=new` returned real, current posts (dozens matching, spanning the prior two days). This is how Monitor's Step 2c sentiment/volume sweep finds community complaint trends instead of relying on the front-page snapshot catching them by chance.
  - **Rate limit (discovered 2026-09-08, hard-enforced):** unauthenticated `reddit.com` calls share ONE per-IP bucket at roughly 1 request per 50-60 seconds — confirmed via live `x-ratelimit-used`/`x-ratelimit-remaining`/`x-ratelimit-reset` response headers (reset values observed between 20s and 54s) and via a real 429 on rapid re-request. The bucket is shared across `.rss` and `search.rss` and across subreddits — a 2026-09-08 23:00 UTC production Monitor run got 429'd after only 2 back-to-back plain `.rss` pulls, skipping the entire sentiment sweep that night. Any code/trigger issuing sequential Reddit calls MUST pace them at least 60 seconds apart and stop (not retry) on a 429.
  - **Multi-subreddit search is real and works:** Reddit's multireddit path (`/r/sub1+sub2/...`) applies to both `.rss` and `search.rss` — `reddit.com/r/pokemongo+TheSilphRoad/search.rss?q=<query>&restrict_sr=on&sort=new` returns results from BOTH subreddits in a single request (verified live 2026-09-08, 25 entries spanning both subs). Use this instead of querying each subreddit separately — it halves the call count for the same coverage.
  - **Boolean OR across search terms does NOT work cleanly:** combining multiple keywords into one query with `OR` (e.g. `q=shiny odds OR broken OR bug OR ...`) returns HTTP 200 but the results degrade toward an unfiltered "new posts" stream rather than staying scoped to the OR'd terms — tested live 2026-09-08, only ~3-4 of 25 returned entries were topically relevant to the intended complaint-sentiment search. Each search term needs its own call; don't try to consolidate terms this way to save on rate-limit budget.
...
| **Reddit `r/sub1+sub2/search.rss?q=<query>&restrict_sr=on&sort=new`** (Atom feed, keyword-scoped, multi-subreddit) | sandbox-blocked | ✅ 200 (verified 2026-09-07/08 — real, current, keyword-matched posts across both subs in one call) | **fetch_url — used by Step 2c's sentiment/volume sweep. Rate-limited to ~1 req/50-60s per IP (shared bucket across all reddit.com calls) — pace calls 60s apart, stop on 429. See rate-limit note above.** |

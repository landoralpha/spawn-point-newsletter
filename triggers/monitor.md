...
**Exception — YouTube sources never reach `Full`:** cap at `Partial` even when a full transcript was fetched for verification. See Step 2c's ethical-use rule.
...
- **YouTube datamine/creator videos** (added 2026-09-18, via Firecrawl MCP — verified working, no login required): 1 call to `firecrawl_search(query="pokemon go datamine <current month/theme>", sources=[{"type":"web"}], includeDomains=["youtube.com"], limit=5)`. If any result looks substantive (dataminer/creator title, not a random clip), `firecrawl_scrape` the top 1-2 candidates with `formats=["markdown"]` to pull the transcript.
  - **Ethical use rule (Joe's explicit preference, 2026-09-18): the transcript is a verification aid for THIS run only, never newsletter raw material.** Read it to judge whether the claimed leak is real and get the facts right (what Pokémon, what mechanic, what date). Do NOT copy or closely paraphrase the creator's own sentences into the Description field, the Notion page body, or (downstream) the newsletter draft. Write the Description in the pipeline's own words, and always name the creator/channel explicitly (e.g. `LiftingandZombies (YouTube) reports...`) — Spawn Point references and credits the video, it does not repurpose it as if it were wire content.
  - **Content Completeness is capped at `Partial`, never `Full`, for any YouTube-sourced entry** — even when the full transcript was fetched. `Full` triggers "pull full article text into page body" elsewhere in this pipeline (see the tier table above), which is the right move for a news article's body text but the wrong move for a creator's own spoken commentary. Discard the transcript text after this run's verification judgment; don't persist it in any Notion field.
  - Source URL = the video URL, so the newsletter's "source-link titles" convention (see `newsletter-creation.md` Source Attribution) naturally links to and credits the video when this entry gets cited.
  - Budget: 1 search + up to 2 scrapes = 3 Firecrawl credits/run. Carry forward as a Step 3 candidate the same way as a datamine account hit (Type `Datamine`, Status `Unconfirmed`). Dedupe against all three keys per Step 1 same as any other source — video leaks often restate a pokeminers.com post, so check Semantic Event Signature carefully before adding a second row for the same leak.

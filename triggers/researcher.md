...
Trigger UUID: ab396bcd-07cf-4f5c-952b-4ecf0a2368ec
Snapshot fetched: 2026-05-11T18:44:44Z
...
### CRITICAL: Premium AND Budget Counters Per Raid Boss
...
Fallback: curl token `ntn_REDACTED_IN_SNAPSHOT` (note: this is a Notion API token routed through the sandbox's allowed Notion endpoint, NOT general curl). The actual token value lives in the live trigger config only — redacted from this git snapshot per GitHub push protection. See live trigger via `RemoteTrigger get trig_01GYjXQqpCgDiFfzo3MKDH5E` if you need the value.

...
| E (Raid schedule) | LeekDuck event pages via fetch_url MCP. Fallback: pokemongo.com/news. | Verify boss is listed in tier on stated dates. Off by ≥1 day → FLAG. Wrong tier → FLAG. **Date-format convention (updated 2026-05-19, see `feedback_raid_rotation_date_convention.md`):** Spawn Point now follows LeekDuck's published date format VERBATIM for Mega and 5-Star rotations. Do NOT normalize, do NOT translate Wed-to-Wed into Wed-Tue, do NOT shift end dates to the preceding Tuesday. If LeekDuck says "Wednesday, May 20 through Wednesday, May 27 at 6:00 AM," that's what Beehiiv copy should say. FLAG any divergence in date phrasing from LeekDuck's published string. (Reversal of earlier May 11 rule. The previous Wed-Tue normalization is no longer in effect.) |
